...
Send the returned assistant message and matching tool result back to the model.
Kimi-K2.6 relies on the original `tool_call.id`; do not replace it with a
synthetic id such as `"call_123"`. When thinking is enabled, also preserve
`reasoning_content` by appending the returned message object or serializing its
`reasoning_content` field into the assistant message.
